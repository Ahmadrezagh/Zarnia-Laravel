...
class Product extends Model implements HasMedia
{
    ...
    {
        // Use the count column if it exists (from migration)
    ...
    {
        return $query->where(function (Builder $q) {
            // Variant product: has etikets but none with is_mojood == 1
            $q->where(function ($variantQuery) {
                $variantQuery->whereNotNull('products.parent_id')
                    // Product has at least one etiket
                    ->whereHas('etikets')
                    // But none of them have is_mojood == 1
